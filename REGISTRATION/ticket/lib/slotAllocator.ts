...
export async function allocateSlot(category: string, dbClient = prisma) {
  // 1. Get or create event settings
  ...
  // 2. Find the previous active registration for this category
  ...
  let duration = settings.musicDuration;
  if (categoryUpper === 'DANCE') {
    duration = settings.danceDuration;
  }
